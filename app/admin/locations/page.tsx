...
interface Seat {
  ...
  row?: string
  column?: number
  type: 'NORMAL' | 'EXCLUSIVE' | 'INSTRUCTOR' | 'PODIUM' | 'ARCHITECTURAL_COLUMN'
  creditCost: number
  x?: number
  y?: number
  isActive: boolean
...
export default function LocationsPage() {
  ...
  const [deleteModal, setDeleteModal] = useState<{ open: boolean; location: Location | null }>({ open: false, location: null })
  ...
  const [editingSeatLayoutId, setEditingSeatLayoutId] = useState<string | null>(null)
  ...
  const [deleteSeatLayoutModal, setDeleteSeatLayoutModal] = useState<{ open: boolean; layout: SeatLayout | null; locationId: string | null }>({ open: false, layout: null, locationId: null })
  const [expandedSeatLayoutId, setExpandedSeatLayoutId] = useState<string | null>(null)
  const [seats, setSeats] = useState<Record<string, Seat[]>>({})
  const [loadingSeats, setLoadingSeats] = useState<Record<string, boolean>>({})
  const [showSeatForm, setShowSeatForm] = useState(false)
  const [editingSeatId, setEditingSeatId] = useState<string | null>(null)
  const [currentSeatLayoutId, setCurrentSeatLayoutId] = useState<string | null>(null)
  const [seatFormData, setSeatFormData] = useState({
    seatNumber: '',
    row: '',
    column: '',
    type: 'NORMAL' as 'NORMAL' | 'EXCLUSIVE' | 'INSTRUCTOR' | 'PODIUM',
    creditCost: 1,
    isActive: true
  })
  const [deleteSeatModal, setDeleteSeatModal] = useState<{ open: boolean; seat: Seat | null; seatLayoutId: string | null }>({ open: false, seat: null, seatLayoutId: null })
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showToast(
        editingId 
          ? (t('location') || 'Location') + ' ' + (t('updated') || 'updated') + ' ' + (t('successfully') || 'successfully')
          : (t('location') || 'Location') + ' ' + (t('created') || 'created') + ' ' + (t('successfully') || 'successfully'),
        'success'
      )
  ...
  const handleDelete = async () => {
    ...
    setSaving(true)
    try {
      const response = await fetch(`/api/locations/${deleteModal.location.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      await fetchLocations(token)
      showToast((t('location') || 'Location') + ' ' + (t('deleted') || 'deleted') + ' ' + (t('successfully') || 'successfully'), 'success')
  ...
  const fetchSeatLayouts = async (locationId: string) => {
    ...
    try {
      const response = await fetch(`/api/locations/${locationId}/seat-layouts`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
      ...
      if (response.ok) {
        const data = await response.json()
        setSeatLayouts(prev => ({ ...prev, [locationId]: Array.isArray(data) ? data : [] }))
      }
    } catch (err) {
  ...
  const handleSaveSeatLayout = async () => {
    ...
    try {
      ...
      if (seatLayoutFormData.gridColumns && seatLayoutFormData.gridColumns.trim()) {
      ...
      showToast(
        editingSeatLayoutId 
          ? (t('seatLayout') || 'Seat Layout') + ' ' + (t('updated') || 'updated') + ' ' + (t('successfully') || 'successfully')
          : (t('seatLayout') || 'Seat Layout') + ' ' + (t('created') || 'created') + ' ' + (t('successfully') || 'successfully'),
        'success'
      )
      setShowSeatLayoutForm(false)
    } catch (err: any) {
  ...
  const handleDeleteSeatLayout = async () => {
    if (!token || !deleteSeatLayoutModal.layout || !deleteSeatLayoutModal.locationId) return

    setSaving(true)
    try {
      const response = await fetch(`/api/seat-layouts/${deleteSeatLayoutModal.layout.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      showToast((t('seatLayout') || 'Seat Layout') + ' ' + (t('deleted') || 'deleted') + ' ' + (t('successfully') || 'successfully'), 'success')
    } catch (err: any) {
  ...
  const fetchSeats = async (seatLayoutId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        setSeats(prev => ({ ...prev, [seatLayoutId]: Array.isArray(data) ? data : [] }))
      }
    } catch (err) {
  ...
  const handleCreateSeat = (seatLayoutId?: string) => {
    ...
    if (!layoutId) {
      showToast(t('selectSeatLayout') || 'Please select a seat layout first', 'error')
      return
    }
    setSeatFormData({
      seatNumber: '',
  ...
  const handleSaveSeat = async () => {
    ...
    try {
      ...
      console.log('Saving seat:', { url, method, seatData, currentSeatLayoutId })
      ...
      showToast(
        editingSeatId 
          ? (t('seat') || 'Seat') + ' ' + (t('updated') || 'updated') + ' ' + (t('successfully') || 'successfully')
          : (t('seat') || 'Seat') + ' ' + (t('created') || 'created') + ' ' + (t('successfully') || 'successfully'),
        'success'
      )
  ...
  const handleDeleteSeat = async () => {
    if (!token || !deleteSeatModal.seat || !deleteSeatModal.seatLayoutId) return
    ...
    try {
      ...
      showToast((t('seat') || 'Seat') + ' ' + (t('deleted') || 'deleted') + ' ' + (t('successfully') || 'successfully'), 'success')
    } catch (err: any) {
    ...
    <div style={{
                  ...
                  {locations.map((location, index) => (
                      ...
                      <td style={{ padding: '1rem' }}>
                        <div style={{ display: 'flex', gap: '0.5rem' }}>
                          <button
                            ...
                            style={{
                              padding: '0.25rem 0.5rem',
                              backgroundColor: expandedLocationId === location.id ? colors.primary : 'transparent',
                              color: expandedLocationId === location.id ? 'white' : colors.primary,
                    ...
                    {expandedLocationId === location.id && (
                      <tr key={`${location.id}-expanded`}>
                        <td colSpan={6} style={{ padding: '1rem', backgroundColor: colors.rowEven }}>
                          <div style={{ padding: '1rem', border: `1px solid ${colors.border}`, borderRadius: '4px' }}>
                            ...
                            {loadingLayouts[location.id] ? (
                              ...
                              <>
                                {seatLayouts[location.id] && seatLayouts[location.id].length > 0 ? (
                                  <div style={{ display: 'grid', gap: '1rem' }}>
                                    {seatLayouts[location.id].map((layout) => (
                                      <div
                                        key={layout.id}
                                        style={{
                                          padding: '1rem',
                                          ...
                                          borderRadius: '4px',
                                          backgroundColor: layout.isActive ? colors.cardBg : colors.rowOdd
                                        }}
                                      >
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '0.5rem' }}>
                                          <div>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem' }}>
                                              <strong style={{ color: colors.text }}>{layout.name}</strong>
                                              {layout.isActive && (
                                                <span style={{
                                            ...
                                            <div style={{ fontSize: '0.875rem', color: colors.textMuted, marginTop: '0.5rem' }}>
                                              {t('seats')}: {layout._count?.seats || layout.seats?.length || 0}
                                              {layout.gridRows && layout.gridColumns && (
                                                <span style={{ marginLeft: '0.5rem', fontWeight: '600' }}>
                                                  • {layout.gridRows}x{layout.gridColumns} {t('grid') || 'grid'}
                                                </span>
                                              )}
                                            </div>
                                          </div>
                                          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                                            <button
                                              onClick={() => toggleSeatLayoutExpanded(layout.id)}
                                              style={{
                                                padding: '0.25rem 0.5rem',
                                                backgroundColor: expandedSeatLayoutId === layout.id ? '#ff9800' : 'transparent',
                                                color: expandedSeatLayoutId === layout.id ? 'white' : '#ff9800',
                                            ...
                                            >
                                              {expandedSeatLayoutId === layout.id 
                                                ? `▼ ${t('seats') || 'Seats'}`
                                                : `▶ ${t('seats') || 'Seats'}`
                                              }
                                            </button>
                                            <button
                                              onClick={() => handleEditSeatLayout(layout)}
                                              style={{
                                                padding: '0.25rem 0.5rem',
                                                backgroundColor: 'transparent',
                                                color: colors.primary,
                                            ...
                                            <button
                                              onClick={() => setDeleteSeatLayoutModal({ open: true, layout, locationId: location.id })}
                                              style={{
                                                padding: '0.25rem 0.5rem',
                                                backgroundColor: 'transparent',
                                                color: colors.error,
                                        ...
                                        {expandedSeatLayoutId === layout.id && (
                                          <div style={{ marginTop: '1rem', padding: '1rem', borderTop: `1px solid ${colors.border}` }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                                              <h4 style={{ margin: 0, color: colors.text, fontSize: '1rem' }}>
                                                {t('seats')} - {layout.name}
                                              </h4>
                                              <button
                                                onClick={() => handleCreateSeat(layout.id)}
                                                style={{
                                                  padding: '0.5rem 1rem',
                                            ...
                                            {loadingSeats[layout.id] ? (
                                              ...
                                              <>
                                                {seats[layout.id] && seats[layout.id].length > 0 ? (
                                                  <div style={{ overflowX: 'auto' }}>
                                                    <table style={{
                                                      width: '100%',
                                                      borderCollapse: 'collapse',
                                                      ...
                                                      <thead>
                                                        <tr style={{
                                                          ...
                                                          <th style={{ padding: '0.5rem', textAlign: 'left', fontWeight: 'bold', color: colors.text }}>
                                                            {t('seatNumber')}
                                                          </th>
                                                          <th style={{ padding: '0.5rem', textAlign: 'left', fontWeight: 'bold', color: colors.text }}>
                                                            {t('row')}
                                                      ...
                                                      <tbody>
                                                        {seats[layout.id].map((seat, seatIndex) => (
                                                          <tr
                                                            key={seat.id}
                                                            style={{
                                                              borderBottom: `1px solid ${colors.border}`,
                                                              backgroundColor: seatIndex % 2 === 0 ? colors.rowEven : colors.rowOdd
                                                            }}
                                                          >
                                                            ...
                                                            <td style={{ padding: '0.75rem' }}>
                                                              <span style={{
                                                                ...
                                                                backgroundColor: 
                                                                  seat.type === 'EXCLUSIVE' 
                                                                    ? (theme === 'dark' ? '#4a148c' : '#f3e5f5')
                                                                    : seat.type === 'INSTRUCTOR'
                                                                    ? (theme === 'dark' ? '#1b5e20' : '#e8f5e9')
                                                                    : seat.type === 'PODIUM'
                                                                    ? (theme === 'dark' ? '#4a148c' : '#e1bee7')
                                                                    : seat.type === 'ARCHITECTURAL_COLUMN'
                                                                    ? (theme === 'dark' ? '#004d40' : '#e0f2f1')
                                                                    : (theme === 'dark' ? '#424242' : '#f5f5f5'),
                                                                color: 
                                                                  seat.type === 'EXCLUSIVE'
                                                                    ? (theme === 'dark' ? '#ce93d8' : '#7b1fa2')
                                                                    : seat.type === 'INSTRUCTOR'
                                                                    ? (theme === 'dark' ? '#81c784' : '#388e3c')
                                                                    : seat.type === 'PODIUM'
                                                                    ? (theme === 'dark' ? '#ba68c8' : '#8e24aa')
                                                                    : seat.type === 'ARCHITECTURAL_COLUMN'
                                                                    ? (theme === 'dark' ? '#4db6ac' : '#00695c')
                                                                    : (theme === 'dark' ? '#b0b0b0' : '#616161')
                                                              }}>
                                                                {seat.type === 'ARCHITECTURAL_COLUMN' 
                                                                  ? (t('architecturalColumn') || 'Architectural Column')
                                                                  : (t(seat.type.toLowerCase()) || seat.type)
                                                                }
                                                              </span>
                                                            </td>
                                                            <td style={{ padding: '0.75rem', color: colors.text }}>
                                                              <strong style={{
                                                                color: seat.type === 'EXCLUSIVE' ? '#e91e63' : colors.text
                                                              }}>
                                                                {seat.type === 'INSTRUCTOR' || seat.type === 'PODIUM' || seat.type === 'ARCHITECTURAL_COLUMN'
                                                                  ? (t('notBookable') || 'Not Bookable')
                                                                  : `${seat.creditCost} ${t('credits') || 'credits'}`
                                                                }
                                                              </strong>
                                                            </td>
                                                            <td style={{ padding: '0.75rem' }}>
                                                            ...
                                                            <td style={{ padding: '0.75rem' }}>
                                                              <div style={{ display: 'flex', gap: '0.5rem' }}>
                                                                ...
                                                                <button
                                                                  onClick={() => setDeleteSeatModal({ open: true, seat, seatLayoutId: layout.id })}
                                                                  style={{
                                                                    padding: '0.25rem 0.5rem',
                                                                    backgroundColor: 'transparent',
                                                                    color: colors.error,
      ...
      >
        <p style={{ color: colors.text, margin: 0 }}>
          {t('deleteWarning') || 'Are you sure you want to delete this location? This action cannot be undone.'}
        </p>
        {deleteModal.location && (
          <p style={{ color: colors.textSecondary, marginTop: '0.5rem', fontWeight: 'bold' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div>
            ...
            <input
              ...
              onChange={(e) => setSeatLayoutFormData({ ...seatLayoutFormData, name: e.target.value })}
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) => setSeatLayoutFormData({ ...seatLayoutFormData, description: e.target.value })}
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setSeatLayoutFormData({ ...seatLayoutFormData, gridRows: e.target.value })}
                  placeholder="e.g., 3"
                  style={{
                    width: '100%',
                    padding: '0.5rem',
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setSeatLayoutFormData({ ...seatLayoutFormData, gridColumns: e.target.value })}
                  placeholder="e.g., 4"
                  style={{
                    width: '100%',
                    padding: '0.5rem',
            ...
            <div style={{ 
              ...
              {t('gridSizeHelp') || 'Optional: Define grid size (e.g., 3 rows x 4 columns = 3x4 grid)'}
            </div>
          </div>
          ...
          <div>
            <label style={{
              ...
              <input
                ...
                onChange={(e) => setSeatLayoutFormData({ ...seatLayoutFormData, isActive: e.target.checked })}
                style={{
                  width: '1rem',
                  height: '1rem',
                  cursor: 'pointer'
      ...
      <Modal
        ...
        onClose={() => setDeleteSeatLayoutModal({ open: false, layout: null, locationId: null })}
      ...
      >
        <p style={{ color: colors.text, margin: 0 }}>
          {t('deleteWarning') || 'Are you sure you want to delete this seat layout? This action cannot be undone.'}
        </p>
        ...
        {(() => {
          ...
          if (currentSeatLayoutId) {
            for (const location of locations) {
              if (seatLayouts[location.id]) {
                currentLayout = seatLayouts[location.id].find(l => l.id === currentSeatLayoutId) || null
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {currentLayout && maxRows && maxColumns && (
                <div style={{
                  ...
                  <strong>{t('gridSize')}:</strong> {maxRows}x{maxColumns} {t('grid')}
                  {' - '}
                  {t('maxRow') || 'Max Row'}: {maxRows}, {t('maxColumn') || 'Max Column'}: {maxColumns}
                </div>
              )}
              ...
              <div>
                <label style={{
                  ...
                  {t('seatNumber')} <span style={{ color: colors.error }}>*</span>
                </label>
                <input
                  type="text"
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                    ...
                    onChange={(e) => {
                      ...
                      if (maxRows && value) {
                        ...
                        if (!isNaN(numValue) && numValue > maxRows) {
                          showToast(`${t('row')} ${t('cannotExceed') || 'cannot exceed'} ${maxRows}`, 'error')
                          return
                        }
                      }
                  ...
                  {maxRows && (
                    <div style={{ fontSize: '0.75rem', color: colors.textMuted, marginTop: '0.25rem' }}>
                      {t('rowHelp') || `Enter row identifier (e.g., A, B, C or 1, 2, 3). Max rows: ${maxRows}`}
                    </div>
                  )}
                </div>
                ...
                <div>
                    ...
                    onChange={(e) => {
                      ...
                      if (maxColumns && value) {
                        ...
                        if (!isNaN(numValue) && numValue > maxColumns) {
                          showToast(`${t('column')} ${t('cannotExceed') || 'cannot exceed'} ${maxColumns}`, 'error')
                          return
                        }
                      }
                  ...
                  {maxColumns && (
                    <div style={{ fontSize: '0.75rem', color: colors.textMuted, marginTop: '0.25rem' }}>
                      {t('columnHelp') || `Enter column number (1 to ${maxColumns})`}
                    </div>
                  )}
                </div>
              </div>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                const newType = e.target.value as 'NORMAL' | 'EXCLUSIVE' | 'INSTRUCTOR' | 'PODIUM' | 'ARCHITECTURAL_COLUMN'
                setSeatFormData({ 
                  ...seatFormData, 
                  type: newType,
                  // Set default credit cost based on type (instructor, podium, and columns are not bookable, cost 0)
                  creditCost: newType === 'EXCLUSIVE' ? 2 : (newType === 'INSTRUCTOR' || newType === 'PODIUM' || newType === 'ARCHITECTURAL_COLUMN') ? 0 : 1
                })
              }}
              style={{
                width: '100%',
            ...
            >
              ...
              <option value="EXCLUSIVE">{t('exclusive') || 'Exclusive'}</option>
              <option value="INSTRUCTOR">{t('instructor') || 'Instructor'}</option>
              <option value="PODIUM">{t('podium') || 'Podium'}</option>
              <option value="ARCHITECTURAL_COLUMN">{t('architecturalColumn') || 'Architectural Column'}</option>
            </select>
            <div style={{ 
              marginTop: '0.5rem', 
              padding: '0.5rem', 
              backgroundColor: 
                seatFormData.type === 'EXCLUSIVE' ? '#fff3e0' :
                seatFormData.type === 'INSTRUCTOR' ? '#e8f5e9' :
                seatFormData.type === 'PODIUM' ? '#f3e5f5' :
                seatFormData.type === 'ARCHITECTURAL_COLUMN' ? '#e0f2f1' :
                '#e3f2fd',
              borderRadius: '4px',
              fontSize: '0.875rem',
              color: colors.textSecondary
              ...
              {seatFormData.type === 'EXCLUSIVE' 
                ? (t('exclusiveSeatInfo') || 'Exclusive seats are premium seats with higher credit cost.')
                : seatFormData.type === 'INSTRUCTOR'
                ? (t('instructorSeatInfo') || 'Instructor position - not bookable by members.')
                : seatFormData.type === 'PODIUM'
                ? (t('podiumSeatInfo') || 'Podium position - elevated platform area.')
                : seatFormData.type === 'ARCHITECTURAL_COLUMN'
                ? (t('architecturalColumnInfo') || 'Architectural column - structural element, not bookable.')
                : (t('normalSeatInfo') || 'Normal seats have standard credit cost.')
              }
            </div>
          </div>
          ...
          <div>
            <label style={{
              ...
              {t('creditCost')} <span style={{ color: colors.error }}>*</span>
            </label>
            <input
              type="number"
              min="0"
              value={seatFormData.creditCost}
              onChange={(e) => setSeatFormData({ ...seatFormData, creditCost: parseInt(e.target.value) || 0 })}
              placeholder={t('creditCost')}
              disabled={seatFormData.type === 'INSTRUCTOR' || seatFormData.type === 'PODIUM' || seatFormData.type === 'ARCHITECTURAL_COLUMN'}
              style={{
                width: '100%',
                padding: '0.5rem',
                border: `1px solid ${colors.inputBorder}`,
                borderRadius: '4px',
                backgroundColor: (seatFormData.type === 'INSTRUCTOR' || seatFormData.type === 'PODIUM' || seatFormData.type === 'ARCHITECTURAL_COLUMN') ? colors.rowOdd : colors.inputBg,
                color: colors.text,
                fontSize: '0.875rem',
                opacity: (seatFormData.type === 'INSTRUCTOR' || seatFormData.type === 'PODIUM' || seatFormData.type === 'ARCHITECTURAL_COLUMN') ? 0.6 : 1,
                cursor: (seatFormData.type === 'INSTRUCTOR' || seatFormData.type === 'PODIUM' || seatFormData.type === 'ARCHITECTURAL_COLUMN') ? 'not-allowed' : 'text'
              }}
            />
            <div style={{ 
              marginTop: '0.25rem', 
              ...
              {t('creditCostHelp') || 'Number of credits required to book this seat'}
            </div>
          </div>
      ...
      <Modal
        ...
        onClose={() => setDeleteSeatModal({ open: false, seat: null, seatLayoutId: null })}
        title={t('deleteSeat')}
        size="small"
      ...
      >
        <p style={{ color: colors.text, margin: 0 }}>
          {t('deleteWarning') || 'Are you sure you want to delete this seat? This action cannot be undone.'}
        </p>
        {deleteSeatModal.seat && (
          <p style={{ color: colors.textSecondary, marginTop: '0.5rem', fontWeight: 'bold' }}>
            {t('seatNumber')}: {deleteSeatModal.seat.seatNumber} ({t(deleteSeatModal.seat.type.toLowerCase()) || deleteSeatModal.seat.type})
          </p>
        )}
      </Modal>
    </div>
