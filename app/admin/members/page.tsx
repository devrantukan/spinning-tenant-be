...
export default function MembersPage() {
  ...
  const [deleteModal, setDeleteModal] = useState<{ isOpen: boolean; memberId: string; memberName: string } | null>(null)
  ...
  const fetchUsers = async (authToken: string, currentMembers: Member[] = members) => {
    try {
    ...
    try {
      ...
      if (formData.createNewUser) {
        ...
        if (!userResponse.ok) {
          ...
          throw new Error(errorData.error || `Failed to create user: HTTP ${userResponse.status}`)
        }
  ...
  const fetchTransactions = async (memberId: string) => {
    ...
    try {
      const response = await fetch(`/api/members/${memberId}/credit-transactions`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
  ...
  const checkInvitationStatuses = async (membersList: Member[], authToken: string) => {
    if (!authToken) {
      console.warn('No auth token, skipping invitation status check')
      return
    }

    console.log(`Checking invitation statuses for ${membersList.length} members`)
    ...
    const statusPromises = membersList
      ...
      .map(async (member) => {
        try {
          ...
          const res = await fetch(`/api/users/${member.userId}/invitation-status`, {
            headers: {
              'Authorization': `Bearer ${authToken}`
            },
          ...
          if (res.ok) {
            ...
            console.error(`Failed to get status for member ${member.userId} (${res.status}):`, errorData)
          ...
          console.error(`Error checking status for member ${member.userId}:`, error)
          return { 
            userId: member.userId, 
            status: { 
              hasInvitation: false, 
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      showToast(t('passwordResetSent') || 'Password reset email sent', 'success')
    } catch (error: any) {
      ...
      {showForm && (
        <div style={{
          ...
          <form onSubmit={(e) => {
            ...
            {editingId ? (
              <>
                <div style={{ marginBottom: '1rem', padding: '1rem', backgroundColor: theme === 'dark' ? '#2a2a2a' : '#f0f7ff', borderRadius: '6px', border: `1px solid ${colors.border}` }}>
                  <h4 style={{ marginTop: 0, marginBottom: '0.75rem', color: colors.text, fontSize: '0.9rem', fontWeight: '600' }}>
                    {t('adjustCreditBalance') || 'Adjust Credit Balance'}
                  </h4>
                  <div style={{ marginBottom: '0.75rem' }}>
                    <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '600', color: colors.text, fontSize: '0.875rem' }}>
                      {t('creditChange') || 'Credit Change'} ({t('positiveToAdd') || 'Positive to add'}, {t('negativeToDeduct') || 'Negative to deduct'})
                    </label>
                    <input
                      type="number"
                      step="0.01"
                  ...
                  <div style={{ marginBottom: '0.75rem' }}>
                    ...
                    <input
                      ...
                      onChange={(e) => setCreditChange({ ...creditChange, description: e.target.value })}
                      placeholder={t('transactionDescriptionPlaceholder') || 'e.g., Payment received, Refund, Adjustment'}
                      style={{
                        width: '100%',
                        padding: '0.5rem',
                        border: `1px solid ${colors.inputBorder}`,
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ backgroundColor: colors.theadBg }}>
                <th style={{ padding: '1rem', textAlign: 'left', borderBottom: `1px solid ${colors.border}` }}>{t('name')}</th>
                <th style={{ padding: '1rem', textAlign: 'left', borderBottom: `1px solid ${colors.border}` }}>{t('email')}</th>
                <th style={{ padding: '1rem', textAlign: 'left', borderBottom: `1px solid ${colors.border}` }}>{t('status')}</th>
                <th style={{ padding: '1rem', textAlign: 'left', borderBottom: `1px solid ${colors.border}` }}>{t('creditBalance') || 'Credit Balance'}</th>
                <th style={{ padding: '1rem', textAlign: 'left', borderBottom: `1px solid ${colors.border}` }}>{t('invitationStatus') || 'Invitation'}</th>
                <th style={{ padding: '1rem', textAlign: 'left', borderBottom: `1px solid ${colors.border}` }}>{t('bookings')}</th>
                <th style={{ padding: '1rem', textAlign: 'left', borderBottom: `1px solid ${colors.border}` }}>{t('actions')}</th>
              </tr>
            </thead>
            <tbody>
                ...
                >
                  ...
                  <td style={{ padding: '1rem', borderBottom: `1px solid ${colors.border}` }}>
                    <span style={{
                      ...
                      borderRadius: '4px',
                      fontSize: '0.875rem',
                      backgroundColor: member.status === 'ACTIVE' ? '#4caf50' : member.status === 'INACTIVE' ? '#ff9800' : '#757575',
                      color: 'white'
                    }}>
                      {member.status}
                    </span>
                  ...
                  <td style={{ padding: '1rem', borderBottom: `1px solid ${colors.border}` }}>
                    {(() => {
                      const invitationStatus = invitationStatuses[member.userId]
                      if (invitationStatus === null || invitationStatus === undefined) {
                        return (
                          <span style={{
                            padding: '0.25rem 0.5rem',
                            borderRadius: '4px',
                  ...
                  <td style={{ padding: '1rem', borderBottom: `1px solid ${colors.border}` }}>
                    <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                      ...
                      {member.userId && (
                        <button
                          onClick={() => handleResetPasswordClick(member.userId, member.user?.email || '')}
                          disabled={resettingPasswords.has(member.userId)}
                          style={{
                            padding: '0.25rem 0.75rem',
                            backgroundColor: resettingPasswords.has(member.userId)
                              ? (theme === 'dark' ? '#555' : '#ccc')
                              : (theme === 'dark' ? '#424242' : '#616161'),
                            color: 'white',
                            border: 'none',
                            borderRadius: '4px',
                            cursor: resettingPasswords.has(member.userId) ? 'not-allowed' : 'pointer',
                            fontSize: '0.875rem',
                            opacity: resettingPasswords.has(member.userId) ? 0.6 : 1,
                            display: 'flex',
                            alignItems: 'center',
                            gap: '0.5rem'
                          }}
                      ...
                      <button
                        onClick={() => handleDeleteClick(member.id, member.user?.name || member.user?.email || 'Member')}
                        style={{
                          padding: '0.25rem 0.75rem',
          ...
          <div style={{
            ...
            {loadingTransactions ? (
              ...
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ backgroundColor: colors.theadBg, borderBottom: `2px solid ${colors.border}` }}>
                      <th style={{ padding: '0.75rem', textAlign: 'left', color: colors.text, fontWeight: '600' }}>
                        {t('date') || 'Date'}
                      </th>
                      <th style={{ padding: '0.75rem', textAlign: 'left', color: colors.text, fontWeight: '600' }}>
                        {t('type') || 'Type'}
                      </th>
                      <th style={{ padding: '0.75rem', textAlign: 'right', color: colors.text, fontWeight: '600' }}>
                        {t('amount') || 'Amount'}
                      </th>
                      <th style={{ padding: '0.75rem', textAlign: 'right', color: colors.text, fontWeight: '600' }}>
                      ...
                      >
                        ...
                        <td style={{ padding: '0.75rem', color: colors.text }}>
                          <span style={{
                            ...
                            fontSize: '0.75rem',
                            backgroundColor:
                              transaction.type === 'MANUAL_ADD' ? (theme === 'dark' ? '#1b5e20' : '#e8f5e9') :
                              transaction.type === 'MANUAL_DEDUCT' ? (theme === 'dark' ? '#5d1a1a' : '#ffebee') :
                              transaction.type === 'BOOKING_PAYMENT' ? (theme === 'dark' ? '#424242' : '#f5f5f5') :
                              transaction.type === 'REFUND' ? (theme === 'dark' ? '#1e3a5f' : '#e3f2fd') :
                              (theme === 'dark' ? '#5d4037' : '#fff3e0'),
                            color:
                              transaction.type === 'MANUAL_ADD' ? (theme === 'dark' ? '#81c784' : '#388e3c') :
                              transaction.type === 'MANUAL_DEDUCT' ? (theme === 'dark' ? '#f48fb1' : '#d32f2f') :
                              transaction.type === 'BOOKING_PAYMENT' ? (theme === 'dark' ? '#b0b0b0' : '#616161') :
                              transaction.type === 'REFUND' ? (theme === 'dark' ? '#90caf9' : '#1976d2') :
