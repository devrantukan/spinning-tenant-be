...
export default function InstructorsPage() {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      if (editForm.specialties !== undefined) updateData.specialties = editForm.specialties
      ...
      setInstructors(instructors.map(i => i.id === editingId ? updated : i))
      setEditingId(null)
      setEditForm({})
    } catch (err: any) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{ margin: 0, fontSize: '1.25rem', color: colors.text }}>{t('instructors')}</h2>
        <button
          onClick={refreshData}
          disabled={loading}
          style={{
            ...
            <table style={{
              ...
              <thead>
                <tr style={{
                  ...
                  <th style={{ padding: '0.75rem', textAlign: 'left', fontWeight: 'bold', color: colors.text }}>{t('status')}</th>
                  <th style={{ padding: '0.75rem', textAlign: 'left', fontWeight: 'bold', color: colors.text }}>{t('bio')}</th>
                  <th style={{ padding: '0.75rem', textAlign: 'left', fontWeight: 'bold', color: colors.text }}>{t('specialties')}</th>
                  <th style={{ padding: '0.75rem', textAlign: 'center', fontWeight: 'bold', color: colors.text }}>{t('classes')}</th>
                  <th style={{ padding: '0.75rem', textAlign: 'center', fontWeight: 'bold', color: colors.text }}>{t('sessions')}</th>
                  <th style={{ padding: '0.75rem', textAlign: 'left', fontWeight: 'bold', color: colors.text }}>{t('created')}</th>
                  <th style={{ padding: '0.75rem', textAlign: 'center', fontWeight: 'bold', color: colors.text }}>{t('actions')}</th>
                </tr>
              </thead>
              <tbody>
                {instructors.map((instructor, index) => (
                  ...
                  >
                    <td style={{ padding: '0.75rem', color: colors.text }}>
                      {instructor.user?.name || <span style={{ color: colors.textMuted, fontStyle: 'italic' }}>{t('noName')}</span>}
                    </td>
                    <td style={{ padding: '0.75rem', color: colors.text }}>{instructor.user?.email || 'N/A'}</td>
                      <td style={{ padding: '0.75rem' }}>
                        <span style={{
                          ...
                          fontSize: '0.75rem',
                          fontWeight: 'bold',
                          backgroundColor: instructor.status === 'ACTIVE' ? '#e8f5e9' : '#ffebee',
                          color: instructor.status === 'ACTIVE' ? '#388e3c' : '#c62828'
                        }}>
                          {instructor.status === 'ACTIVE' ? t('active') : t('inactive')}
                        </span>
                      </td>
                      <td style={{ padding: '0.75rem', maxWidth: '200px', color: colors.text }}>
                        {instructor.bio ? (
                          <span style={{
                            display: 'block',
                            overflow: 'hidden',
                          ...
                          <span style={{ color: colors.textMuted, fontStyle: 'italic' }}>{t('noBio')}</span>
                        )}
                      </td>
                      <td style={{ padding: '0.75rem' }}>
                        {instructor.specialties && instructor.specialties.length > 0 ? (
                          ...
                          <span style={{ color: colors.textMuted, fontStyle: 'italic' }}>{t('none')}</span>
                        )}
                      </td>
                      ...
                      <td style={{ padding: '0.75rem', textAlign: 'center' }}>
                        <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center' }}>
                          <button
                            ...
                            disabled={editingId === instructor.id || deletingId === instructor.id}
                            style={{
                              padding: '0.25rem 0.5rem',
                              ...
                              border: 'none',
                              borderRadius: '4px',
                              cursor: editingId === instructor.id || deletingId === instructor.id ? 'not-allowed' : 'pointer',
                              fontSize: '0.75rem',
                              opacity: editingId === instructor.id || deletingId === instructor.id ? 0.5 : 1
                            }}
                          >
                              {t('edit')}
                          </button>
                          ...
                          >
                            {deletingId === instructor.id ? t('deleting') : t('delete')}
                          </button>
                        </div>
                      </td>
                    </tr>
                    {editingId === instructor.id && (
                      <tr key={`${instructor.id}-edit`}>
                        <td colSpan={9} style={{ padding: '1rem', backgroundColor: '#f9f9f9' }}>
                          <div style={{
                            padding: '1rem',
                            ...
                            <h4 style={{ marginTop: 0, marginBottom: '1rem' }}>{t('editInstructor')}</h4>
